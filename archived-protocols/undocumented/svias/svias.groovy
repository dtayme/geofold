    ...
    variant("main") {
        ...
        encode { cmd, ctx ->
            switch (cmd.type) {
                ...
                case TYPE_POSITION_SINGLE: return "AT+STR=1*"
                case TYPE_SET_ODOMETER:    return "AT+ODT=${ctx.data()}*"
                case TYPE_ENGINE_STOP:     return "AT+OUT=1,1*"
                case TYPE_ENGINE_RESUME:   return "AT+OUT=1,0*"
                case TYPE_ALARM_ARM:       return "AT+OUT=2,1*"
                case TYPE_ALARM_DISARM:    return "AT+OUT=2,0*"
                case TYPE_ALARM_REMOVE:    return "AT+PNC=600*"
                default:                   return null
            }
        }
    }
